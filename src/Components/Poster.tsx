...
export default function Poster({ item, index, translateX }: PosterProps) {
    const INPUT_RANGE = [(index - 1) * Device.width, index * Device.width, (index + 1) * Device.width];

    const rStyledPoster = useAnimatedStyle(() => {
        return {
            transform: [{ translateY: withSpring(interpolate(translateX.value, INPUT_RANGE, [-100,0,-100]), {
                velocity: 2500,
                damping: 72
            }) }],
            opacity: interpolate(translateX.value, INPUT_RANGE, [0, 1, 0.5])
        }
    });

    const hStyledPoster = useAnimatedStyle(() => {
        return {
            transform: [{ translateY: withSpring(interpolate(translateX.value, INPUT_RANGE, [80,0,80]), {
                velocity: 2500,
                damping: 72
            }) }],
